...
def parse_command(text: str) -> dict:
    ...
    if any(word in text for word in ["폴더", "디렉토리", "찾", "열", "보여"]):
        if "파일" not in text and "앱" not in text:
            match = re.search(r"(.*?)(폴더|디렉토리|찾|열|보여)", text)
            if match:
                target = match.group(1).strip()
                return {"intent": "디렉토리 찾기", "target": target}

    return {"intent": "알 수 없음", "target": text}
